...
export const get_by_doi = tool({
  ...
  args: {
    doi: tool.schema.string().describe("论文 DOI（不含 https://doi.org/ 前缀）"),
  },
  async execute(args) {
    const doi = args.doi.trim().replace(/^https?:\/\/doi\.org\//, "")

    const [cr, s2, oa] = await Promise.allSettled([
      getByDoiCrossref(doi),
      getByDoiSemantic(doi),
      getByOpenAlex(doi),
    ])
...
export const get_by_s2id = tool({
  description: "通过 Semantic Scholar Paper ID 获取论文详情。适用于搜索结果返回的 S2 paperId。",
  args: {
    paperId: tool.schema.string().describe("Semantic Scholar Paper ID（如 CorpusId:12345 或 40f3c5f8c3e2d9e1...）"),
  },
  async execute(args) {
    const result = await getByS2Id(args.paperId)
    if (!result) {
...
export default tool({
  description: "通过 DOI 获取论文完整详情（多源合并）。优先使用此工具获取论文摘要、作者、引用数等完整信息。",
  args: {
    doi: tool.schema.string().describe("论文 DOI（不含 https://doi.org/ 前缀）"),
  },
  async execute(args) {

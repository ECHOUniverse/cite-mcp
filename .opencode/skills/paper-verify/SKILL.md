...
| 工具名 | 用途 |
|--------|------|
| `paper-search` (default export) | 多源聚合搜索（OpenAlex + Semantic Scholar + Crossref），推荐优先使用 |
| `paper-search_search_openalex` | 单独搜索 OpenAlex |
| `paper-search_search_semantic` | 单独搜索 Semantic Scholar |
...
- 优先使用 `paper-search`（多源聚合搜索）
- 搜索关键词：论点核心概念的**英文**关键词
- 若结果不足（<3篇），补充使用 `paper-search_search_semantic` 或 `paper-search_search_openalex` 单源搜索
